...
// #hits per pc, #hits per pc for fill accuracy

std::map<uint64_t, uint64_t> pcHitCount;
std::map<uint64_t, uint64_t> pcFillCount;
#define BT 0.2
...
void CACHE::handle_fill()
{
  while (writes_available_this_cycle > 0) {
    ...
    #endif
    bool bypassCondition;
    bypassCondition = ( 
      warmup_complete[fill_mshr->cpu] && 
      check_string(NAME,"LLC") && pcFillCount[fill_mshr->ip] != 0 &&
      (double)(pcHitCount[fill_mshr->ip]) 
      / pcFillCount[fill_mshr->ip] <BT );
    if(bypassCondition){
...
void CACHE::handle_writeback()
{
  while (writes_available_this_cycle > 0) {
    ...
    bool bypassCondition;
    bypassCondition = ( 
      warmup_complete[handle_pkt.cpu] && 
      check_string(NAME,"LLC") && pcFillCount[handle_pkt.ip] != 0 &&
      (double)(pcHitCount[handle_pkt.ip]) 
      / pcFillCount[handle_pkt.ip] <BT 
      );
    if(bypassCondition){
      WQ.pop_front();
      writes_available_this_cycle--;
  ...
  while (reads_available_this_cycle > 0) {
    ...
    if (way < NUM_WAY) // HIT
    {
      ...
      if(!warmup_complete[handle_pkt.cpu]
        && check_string(NAME, "LLC")
      ){
          pcHitCount[handle_pkt.ip]++;
      }
